...
class ProjectConfig(BaseModel):
    ...
    key: str = "ds-forecasting-sklearn"
    name: str = "electricity-load-forecasting-sklearn"
    domain: str = "data-science"
    problem: str = "time-series-forecasting"
    stack: str = "sklearn"
    family: str = "time_series_forecasting"
    modality: str = "tabular"
    title: str = "Prévision de consommation électrique multi-horizons avec scikit-learn"
    version: str = "1.0.0"
...
class DataConfig(BaseModel):
    ...
    dataset_name: str = "regional_electricity_load"
    n_samples: int = Field(default=4800, ge=50)
    seed: int = 42
    formats: list[str] = Field(default_factory=lambda: ["parquet", "csv"])
    raw_dir: Path = Path("data/raw")
    processed_dir: Path = Path("data/processed")
    external_dir: Path = Path("data/external")
    target: str | None = "load_mw"
    id_column: str | None = "sample_id"
    time_column: str | None = "origin_date"
...
class ModelConfig(BaseModel):
    ...
    name: str = "Gradient boosting par histogrammes (HistGradientBoostingRegressor)"
    algorithm: str = "hist_gradient_boosting"
    model_class: str = Field(default="SklearnModel", alias="class")
...
class MetricsConfig(BaseModel):
    ...
    task: Task = "forecasting"
    primary: str = "mape"
    secondary: list[str] = Field(default_factory=lambda: ["smape", "mase", "mae", "rmse", "r2"])
    baseline: str = "dummy"
    direction: Literal["maximize", "minimize"] = "minimize"
    min_primary: float | None = 4.0

...
class ForecastingPlots:
    ...
    def forecast_vs_actual(
        ...
        for column, label in (("load_seasonal_naive", "naif saisonnier"),):
            if column in plot_frame.columns:
                axis.plot(
                    stamps, plot_frame[column], color=COLOR_NAIVE, lw=1.0, ls=":", label=label
                )
    ...
    def error_by_horizon(self, result: Any, *, name: str = "error_by_horizon.png") -> Path | None:
        ...
        axis.set_title(
            "Erreur par horizon — le modèle doit rester sous la référence à chaque horizon"
        )
    ...
    def improvement_vs_baselines(
        ...
        axis.set_title(
            "Valeur ajoutée réelle : ce que le modèle apporte face aux références triviales"
        )
    ...
    def error_breakdown(self, result: Any, *, name: str = "error_breakdown.png") -> Path | None:
        ...
        for axis, name_ in zip(axes_list, axes_names, strict=False):
            ...
            axis.barh(
                block["segment"].astype(str), block["mape_pct"].astype(float), color="#4472c4"
            )
            axis.invert_yaxis()
    ...
    def metrics_bar(self, result: Any, *, name: str = "metrics_bar.png") -> Path | None:
        ...
        selected = {
            ...
            if key in {"mape", "smape", "mase", "interval_coverage"}
            and np.isfinite(_safe_float(value))
        }
        if not selected:
            return None
        fig, axis = plt.subplots(figsize=(6.4, 3.4))
        axis.bar(
            list(selected), [_safe_float(value) for value in selected.values()], color="#4472c4"
        )

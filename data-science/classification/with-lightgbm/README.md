...
> **Domaine** : `data-science` · **Problématique** : `classification` · **Stack** : `LightGBM`
> **Emplacement** : `data-science/classification/with-lightgbm/`
...
-> gradient boosting leaf-wise LightGBM avec early stopping natif -> évaluation et analyse
...
1. Comprendre la croissance leaf-wise (`num_leaves`) et son risque de sur-apprentissage face à `max_depth`.
2. Armer l'early stopping natif via un callback LightGBM (`early_stopping`, `log_evaluation`) sans réécrire la boucle.
3. Régulariser un booster (min_child_samples, subsample + subsample_freq, colsample_bytree, reg_lambda, min_split_gain).
4. Exploiter le support natif des variables catégorielles plutôt que le one-hot encoding systématique.
5. Traiter un déséquilibre de classes avec `scale_pos_weight` calculé depuis les données.
6. Comparer gbdt / dart / goss et des baselines honnêtes sur un même split de validation.
...
```text
data-science/classification/with-lightgbm/
...
```bash
cd data-science/classification/with-lightgbm
python -m venv .venv
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/classification/with-lightgbm` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-classification-lightgbm`*

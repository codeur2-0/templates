...
> **Domaine** : `data-science` · **Problématique** : `classification` · **Stack** : `XGBoost`
> **Emplacement** : `data-science/classification/with-xgboost/`
...
-> gradient boosting régulé XGBoost avec early stopping natif -> évaluation et analyse d'erreurs)
pour un cas de churn télécom, entièrement piloté par Hydra et structuré en objets testables.
...
3. Régulariser un booster (max_depth, min_child_weight, subsample, colsample_bytree, reg_lambda, gamma).
4. Traiter un déséquilibre de classes avec `scale_pos_weight` plutôt qu'avec du sur-échantillonnage.
...
```text
data-science/classification/with-xgboost/
...
```bash
cd data-science/classification/with-xgboost
python -m venv .venv
...
python -m src.main mode=train model.params.n_estimators=400
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/classification/with-xgboost` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-classification-xgboost`*

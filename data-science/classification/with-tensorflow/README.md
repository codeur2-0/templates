# Prédiction d'attrition client (churn télécom) avec TensorFlow
...
> **Domaine** : `data-science` · **Problématique** : `classification` · **Stack** : `TensorFlow`
> **Emplacement** : `data-science/classification/with-tensorflow/`
...
-> réseau subclassé entraîné au GradientTape sur un pipeline tf.data -> évaluation et analyse
...
1. Écrire une boucle d'entraînement TensorFlow bas niveau : tf.data, GradientTape, apply_gradients.
2. Compiler le pas d'entraînement avec tf.function et comprendre le retraçage (reduce_retracing).
3. Subclasser keras.Model et keras.layers.Layer : call(features, training=...), get_config().
4. Ajouter les pertes de régularisation (L2) à la perte métier et écrêter la norme du gradient.
5. Travailler sur logits : entropie croisée from_logits=True et sigmoid/softmax côté inférence.
6. Pondérer les échantillons pour traiter un déséquilibre de classes (équivalent du pos_weight).
7. Paramétrer un ordonnanceur de taux d'apprentissage en pas d'optimiseur, pas en époques.
8. Persister un modèle subclassé : poids (.weights.h5) + configuration JSON, reconstruction au chargement.
...
```text
data-science/classification/with-tensorflow/
...
```bash
cd data-science/classification/with-tensorflow
python -m venv .venv
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/classification/with-tensorflow` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-classification-tensorflow`*

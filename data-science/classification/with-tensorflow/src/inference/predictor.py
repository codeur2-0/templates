"""Inference: score new customers with the trained artefacts.
...
4. returns a business-readable frame: probability, decision, risk segment, main drivers.
...
#: Segments de risque attribués selon la probabilité prédite.
RISK_SEGMENTS: tuple[tuple[float, str], ...] = (
    (0.20, "faible"),
    (0.50, "modéré"),
    (0.75, "élevé"),
    (1.01, "critique"),
...
class Predictor:
    """Batch and single-record prediction, with validation and light explainability."""
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            ValueError: When the threshold is outside ``[0, 1]``.
        """
    ...
    def from_config(cls, config: Any, paths: ProjectPaths | None = None) -> Predictor:
        ...
        threshold = float(((payload.get("predict") or {}).get("threshold")) or 0.5)

        logger.info(
            "Predictor ready | model={} features={} threshold={}",
            model.summary(),
    ...
    def predict(self, frame: pd.DataFrame, *, validate: bool = True) -> pd.DataFrame:
        ...
        Returns:
            A frame with the context columns, ``probability``, ``prediction``,
            ``risk_segment`` and ``decision_reason``.
        ...
        if probabilities is not None:
            ...
            output["prediction"] = (probabilities >= self.threshold).astype(int)
        else:
            output["probability"] = np.nan
            output["prediction"] = np.asarray(predictions).astype(int)
        output["risk_segment"] = (
            output["probability"].map(_risk_segment) if probabilities is not None else "n/a"
        )
        output["decision_reason"] = output.apply(self._decision_reason, axis=1)

        flagged = int(output["prediction"].sum())
        logger.info(
            "Predictions | rows={} flagged={} ({:.1%}) mean_probability={:.4f}",
            len(output),
            flagged,
            flagged / max(len(output), 1),
            float(output["probability"].mean()) if probabilities is not None else float("nan"),
        )
        return output

    def predict_one(self, record: Mapping[str, Any]) -> dict[str, Any]:
        """Score a single record and return a JSON-serialisable payload.

        Args:
            record: Mapping of column name to value.

        Returns:
            The prediction payload (probability, decision, segment, drivers).
        """
        ...
        payload = {
            ...
            "probability": _jsonable(row.get("probability")),
            "prediction": int(row.get("prediction", 0)),
            "risk_segment": str(row.get("risk_segment", "n/a")),
    ...
    def top_drivers(
        ...
        """Return the features that push the prediction the most for one record.

        The contribution is a *local* approximation: feature importance (global) multiplied by
        the standardised value of the record. It is meant for operational explainability, not
        for attribution exacte — use SHAP when a formal explanation is required.
    ...
    # ------------------------------------------------------------------ internals -------
    def _probabilities(self, matrix: pd.DataFrame) -> np.ndarray | None:
        """Return the positive-class probability when available."""
        if not getattr(self.model, "supports_proba", False):
            return None
        try:
            probabilities = np.asarray(self.model.predict_proba(matrix))
        except (NotImplementedError, AttributeError) as exc:
            logger.debug("Probabilities unavailable: {}", exc)
            return None
    ...
    def _decision_reason(self, row: pd.Series) -> str:
        ...
        if probability is None or (isinstance(probability, float) and np.isnan(probability)):
            return "probabilité indisponible (le modèle n'expose pas de proba)"
        decision = "à risque de départ" if float(probability) >= self.threshold else "fidèle"
        return f"p={float(probability):.3f} (seuil {self.threshold:.2f}) → {decision}"


def _risk_segment(probability: Any) -> str:
    """Map a probability onto a business risk segment."""
    try:
    ...
    if np.isnan(value):
        return "n/a"
    for upper_bound, label in RISK_SEGMENTS:
        if value < upper_bound:
            return label
    return RISK_SEGMENTS[-1][1]

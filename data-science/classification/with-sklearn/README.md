...
> **Domaine** : `data-science` · **Problématique** : `classification` · **Stack** : `scikit-learn`
> **Emplacement** : `data-science/classification/with-sklearn/`
...
-> forêt aléatoire -> évaluation et analyse d'erreurs) pour un cas de churn télécom, entièrement
piloté par Hydra et structuré en objets testables.
...
3. Lire des métriques de classification en contexte déséquilibré (ROC AUC, PR AUC, rappel, précision).
4. Choisir un seuil de décision avec une table d'arbitrage coût/bénéfice plutôt qu'avec 0.5.
5. Produire une importance de features et l'interpréter sans sur-vendre la causalité.
...
```text
data-science/classification/with-sklearn/
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/classification/with-sklearn` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-classification-sklearn`*

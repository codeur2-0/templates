# Prédiction d'attrition client (churn télécom) avec Keras
...
> **Domaine** : `data-science` · **Problématique** : `classification` · **Stack** : `Keras (API fonctionnelle)`
> **Emplacement** : `data-science/classification/with-keras/`
...
-> réseau de neurones déclaré avec l'API fonctionnelle Keras puis compilé et entraîné -> évaluation
et analyse d'erreurs) pour un cas de churn télécom, entièrement piloté par Hydra et structuré en
objets testables.
...
1. Déclarer un réseau avec l'API fonctionnelle Keras (Input -> couches -> Model) puis le compiler.
2. Choisir la bonne activation de sortie et la bonne perte selon la tâche (sigmoid/BCE, softmax/CE sparse, linear/MSE).
3. Utiliser les callbacks natifs : EarlyStopping(restore_best_weights), ReduceLROnPlateau, TerminateOnNaN.
4. Brancher les callbacks du projet sur la boucle Keras via un pont (`keras.callbacks.Callback`).
5. Traiter un déséquilibre de classes avec `fit(class_weight=...)` et comprendre la formule 'balanced'.
6. Ordonnancer le taux d'apprentissage : schedules (cosine, exponential) dans l'optimiseur vs callback plateau.
7. Sauvegarder au format natif `.keras` et documenter les méta-données métier dans un sidecar JSON.
8. Garantir la reproductibilité : graines Keras, déterminisme des opérations TensorFlow.
...
```text
data-science/classification/with-keras/
...
```bash
cd data-science/classification/with-keras
python -m venv .venv
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/classification/with-keras` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-classification-keras`*

...
> **Domaine** : `data-science` · **Problématique** : `regression` · **Stack** : `PyTorch`
> **Emplacement** : `data-science/regression/with-pytorch/`
...
-> réseau de neurones PyTorch entraîné par descente de gradient explicite -> analyse des résidus par
...
2. Normaliser une cible continue en interne : pourquoi un MSE sur des montants en euros diverge et comment l'éviter.
3. Comprendre ce que coûte le deep learning sur données tabulaires et quand lui préférer un booster.
4. Régulariser un réseau : dropout, weight decay, batch norm, early stopping avec restauration du meilleur état.
5. Ordonnancer le taux d'apprentissage (cosine, plateau) et lire son effet sur les courbes d'apprentissage.
6. Garantir le déterminisme : graine avant initialisation, mélange généré par un Generator semé, CPU.
7. Sauvegarder un checkpoint (poids + métadonnées, y compris les paramètres de cible) et le recharger sans pickler l'objet Python.
...
```text
data-science/regression/with-pytorch/
...
```bash
cd data-science/regression/with-pytorch
python -m venv .venv
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/regression/with-pytorch` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-17 · projet `ds-regression-pytorch`*

"""Model factory of the **PyTorch** stack.
...
* :func:`build_model` reads the ``model`` node (algorithm, hyper-parameters, seed) and returns a
  :class:`~src.models.model.PyTorchModel` respecting the :class:`~src.models.base.BaseModel`
  contract,
...
PROJECT_TASK = "regression"

#: Algorithme retenu dans `conf/model/default.yaml`.
CONFIGURED_ALGORITHM = "mlp"
...
def load_model(path: str | Path, *, config: Mapping[str, Any] | None = None) -> BaseModel:
    ...
    if source.suffix.lower() not in SUPPORTED_SUFFIXES:
        msg = (
            f"Unsupported model artefact '{source.name}' for the PyTorch stack "
            f"(expected one of {sorted(set(SUPPORTED_SUFFIXES))})"
        )
        raise ValueError(msg)
    return PyTorchModel.load(source, config=config)

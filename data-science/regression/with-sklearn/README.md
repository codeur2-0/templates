...
> **Domaine** : `data-science` · **Problématique** : `regression` · **Stack** : `scikit-learn`
> **Emplacement** : `data-science/regression/with-sklearn/`
...
-> gradient boosting histogrammé -> analyse des résidus par segment) pour un modèle d'évaluation
automatisée de prix immobilier, entièrement piloté par Hydra et structuré en objets testables.
L'estimation publiée est une fourchette assortie d'un niveau de confiance.
...
1. Composer un pipeline de régression scikit-learn propre : ColumnTransformer, transformers custom, fit sur le train uniquement.
2. Exploiter la gestion native des manquants d'un boosting histogrammé, et la comparer à une imputation explicite.
3. Utiliser une classe abstraite BaseModel pour rendre le framework interchangeable.
4. Lire des métriques de régression en contexte hétéroscédastique (RMSE, MAE, MAPE, R2, erreur maximale).
5. Diagnostiquer un biais segmenté : la RMSE globale masque une sous-estimation systématique d'un quartier.
6. Publier une fourchette et un niveau de confiance plutôt qu'un prix unique, et mesurer la couverture.
7. Produire une importance de features et l'interpréter sans sur-vendre la causalité.
...
```text
data-science/regression/with-sklearn/
...
```bash
cd data-science/regression/with-sklearn
python -m venv .venv
...
python -m src.main mode=train model.params.max_iter=400
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/regression/with-sklearn` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |

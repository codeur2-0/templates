"""Model factory of the **Keras (API fonctionnelle)** stack.

L'API fonctionnelle montre l'autre façon de faire du deep learning : le graphe est **déclaré**
(entrées -> couches -> modèle), puis **compilé** avec son optimiseur, sa perte (erreur
quadratique moyenne sur une cible continue normalisée en interne) et ses métriques, avant que
`fit` ne prenne en charge la boucle d'entraînement. On y gagne l'inspection du réseau, la
sérialisation native (.keras) et les callbacks éprouvés (EarlyStopping, ReduceLROnPlateau,
...
* :func:`build_model` reads the ``model`` node (algorithm, hyper-parameters, seed) and returns a
  :class:`~src.models.model.KerasModel` respecting the :class:`~src.models.base.BaseModel`
  contract,
...
def load_model(path: str | Path, *, config: Mapping[str, Any] | None = None) -> BaseModel:
    ...
    if source.suffix.lower() not in SUPPORTED_SUFFIXES:
        msg = (
            f"Unsupported model artefact '{source.name}' for the Keras (API fonctionnelle) stack "
            f"(expected one of {sorted(set(SUPPORTED_SUFFIXES))})"
        )
        raise ValueError(msg)
    return KerasModel.load(source, config=config)

# Estimation du prix de vente immobilier (AVM) avec Keras (API fonctionnelle)

![Python](https://img.shields.io/badge/python-3.10%2B-blue?logo=python&logoColor=white)
![Keras (API fonctionnelle)](https://img.shields.io/badge/Keras%20(API%20fonctionnelle)-deep-learning-orange)
...
> **Domaine** : `data-science` · **Problématique** : `regression` · **Stack** : `Keras (API fonctionnelle)`
> **Emplacement** : `data-science/regression/with-keras/`
...
-> réseau de neurones Keras déclaré puis compilé (API fonctionnelle) -> analyse des résidus par
...
1. Normaliser une cible continue en interne : pourquoi un MSE sur des montants en euros diverge et comment l'éviter.
2. Comprendre ce que coûte le deep learning sur données tabulaires et quand lui préférer un booster.
3. Régulariser un réseau : dropout, weight decay, batch norm, early stopping avec restauration du meilleur état.
4. Ordonnancer le taux d'apprentissage (cosine, plateau) et lire son effet sur les courbes d'apprentissage.
5. Garantir le déterminisme : graine avant construction du graphe, mélange semé, CPU.
6. Déclarer un graphe (entrées -> couches -> modèle) puis le compiler avec optimiseur, perte et métriques.
7. Sauvegarder au format natif .keras (poids + architecture) avec un sidecar JSON de métadonnées de projet.

**Pourquoi Keras (API fonctionnelle) ici ?**

- API fonctionnelle déclarative : le graphe du réseau est construit puis compilé (`compile/fit`).
- Callbacks natifs : `EarlyStopping`, `ReduceLROnPlateau`, `ModelCheckpoint`.
...
| --- | --- | --- | --- |
| Framework ML | **Keras (API fonctionnelle)** | tensorflow-cpu>=2.16 | deep-learning |
...
```text
data-science/regression/with-keras/
...
│   ├── models/                  # BaseModel (ABC) + implémentation Keras (API fonctionnelle)
...
AppConfig (Pydantic)                 ← conf/*.yaml validé et typé
   ...
   │
   ├── KerasModel(BaseModel)   ← implémentation Keras (API fonctionnelle)
...
```bash
cd data-science/regression/with-keras
python -m venv .venv
...
python -m src.main mode=train model.params.hidden_layers=[128, 64]
...
from src.models.model import KerasModel
...
| `conf/config.yaml` -> bloc `estimation` | Réglages métier de la famille, au même niveau que `mode` et `seed` : 4 clés (tolerance_pct, depth_column, unit_column, min_market_depth), chacune commentée dans le fichier — c'est là qu'on change le métier sans toucher au code. | `estimation.tolerance_pct=10.0`, `estimation.depth_column=recent_sales_1km` |
...
| Chemin | Contenu |
| --- | --- |
| `data/raw/real_estate_prices.parquet` (+ `csv`) | jeu de données synthétique, 8 000 lignes |
...
| `artifacts/models/model.keras` | modèle entraîné |
...
| `artifacts/reports/top_errors.csv` | résidus les plus élevés, avec leurs features |
...
Métrique principale : **`rmse`** (sens `minimize`, seuil de smoke test : ≤ 90000.0).
...
| `04_model_exploration.ipynb` | *Exploration et comparaison de modèles* — Baseline d'abord — sans elle aucune performance n'est interprétable — puis comparaison des algorithmes de la stack, distinction sous-apprentissage / sur-apprentissage par la courbe d'apprentissage, et lecture de l'importance des features. |
...
| `06_error_analysis.ipynb` | *Analyse d'erreurs et recommandations* — Métriques globales, résidus et couverture de fourchette, détection d'un **biais segmenté** (le modèle se trompe-t-il toujours dans le même sens ?), pires erreurs et leur cause probable, puis recommandations appuyées sur les chiffres observés. |
...
- **Reproductibilité** : graine propagée à Python/NumPy/Keras (API fonctionnelle), données régénérables à l'identique.
...
| `src/models/` | `BaseModel` (ABC) + implémentation Keras (API fonctionnelle) | Aucune logique de training loop ici : le modèle expose un contrat. |
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/regression/with-keras` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
| `Could not find a version that satisfies the requirement keras (api fonctionnelle)` | Index PyPI inaccessible / Python trop ancien | Python ≥ 3.10, `pip install --upgrade pip`, vérifier le proxy |
...
*Dernière génération : 2026-09-17 · projet `ds-regression-keras`*

...
def _diagnostic_frame(result: Any) -> pd.DataFrame:
    ...
    if "y_true" in frame.columns:
        ...
        frame["within_tolerance"] = (frame["relative_error_pct"].abs() <= TOLERANCE_PCT).astype(
            "int64"
        )
    return frame
...
class RegressionPlots:
    ...
    def predicted_vs_actual(
        ...
        axes.plot(
            span, span * (1 - TOLERANCE_PCT / 100.0), color="grey", linestyle="--", linewidth=1.0
        )
    ...
    def residuals_vs_predicted(
        ...
        try:
            frame["bucket"] = pd.qcut(
                frame["prediction"], q=min(DEFAULT_BUCKETS, 8), duplicates="drop"
            )
        except ValueError:
            frame["bucket"] = "all"
    ...
    def error_distribution(
        self, result: Any, *, name: str = "error_distribution.png"
    ) -> Path | None:
        ...
        if relative is None:
            return None
        values = (
            pd.to_numeric(pd.Series(relative), errors="coerce").dropna().to_numpy(dtype="float64")
        )
        if values.size == 0:
            return None
        ...
        axes.axvline(
            median, color="crimson", linestyle="--", linewidth=1.4, label=f"médiane {median:+.2f} %"
        )
        axes.axvline(
            p95, color="darkorange", linestyle=":", linewidth=1.4, label=f"P95 |erreur| {p95:.2f} %"
        )
    ...
    def coverage_by_bucket(
        self, result: Any, *, name: str = "coverage_by_bucket.png"
    ) -> Path | None:
        ...
        axes.axhline(
            70.0, color="crimson", linestyle="--", linewidth=1.4, label="cible métier (70 %)"
        )
    ...
    def feature_importance(
        ...
        axes.barh(
            ...
            data["importance"].astype("float64"),
            color="teal",
            alpha=0.85,
        )
    ...
    def worst_errors(
        self, result: Any, *, name: str = "worst_errors.png", top_k: int = 10
    ) -> Path | None:
        ...
        if errors is None or len(errors) == 0:
            return None
        frame = (
            errors.head(int(top_k)) if isinstance(errors, pd.DataFrame) else pd.DataFrame(errors)
        )

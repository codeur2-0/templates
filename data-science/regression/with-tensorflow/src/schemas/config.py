...
class ProjectConfig(BaseModel):
    ...
    key: str = "ds-regression-tensorflow"
    name: str = "real-estate-avm-tensorflow"
    domain: str = "data-science"
    problem: str = "regression"
    stack: str = "tensorflow"
    family: str = "regression"
    modality: str = "tabular"
    title: str = "Estimation du prix de vente immobilier (AVM) avec TensorFlow (GradientTape)"
    version: str = "1.0.0"
...
class DataConfig(BaseModel):
    ...
    dataset_name: str = "real_estate_prices"
    n_samples: int = Field(default=8000, ge=50)
    seed: int = 42
    formats: list[str] = Field(default_factory=lambda: ["parquet", "csv"])
    raw_dir: Path = Path("data/raw")
    processed_dir: Path = Path("data/processed")
    external_dir: Path = Path("data/external")
    target: str | None = "price_eur"
    id_column: str | None = "property_id"
    time_column: str | None = None
    group_column: str | None = "district"
...
class ArtifactsConfig(BaseModel):
    ...
    model_file: str = "model.weights.h5"
...
class TrainConfig(BaseModel):
    ...
    epochs: int = Field(default=45, ge=1)
...
class ModelConfig(BaseModel):
    ...
    name: str = "Réseau TensorFlow subclassé (GradientTape)"
    algorithm: str = "mlp"
    model_class: str = Field(default="TensorFlowModel", alias="class")
    task: Task = "regression"
    supports_proba: bool = True
    epochs_based: bool = True
    random_state: int = 42
...
class MetricsConfig(BaseModel):
    ...
    secondary: list[str] = Field(
        default_factory=lambda: ["mae", "r2", "mape", "smape", "max_error"]
    )
    baseline: str = "dummy"
    direction: Literal["maximize", "minimize"] = "minimize"
    min_primary: float | None = 90000.0

# Estimation du prix de vente immobilier (AVM) avec TensorFlow (GradientTape)
...
> **Domaine** : `data-science` · **Problématique** : `regression` · **Stack** : `TensorFlow`
> **Emplacement** : `data-science/regression/with-tensorflow/`
...
-> réseau TensorFlow subclassé entraîné pas à pas via GradientTape -> analyse des résidus par
...
6. Écrire une boucle d'entraînement bas niveau : tf.data.Dataset, tf.GradientTape, apply_gradients, tf.function.
7. Écrêter le gradient (`clip_norm`) et comprendre son rôle quand la perte n'est pas bornée.
...
```text
data-science/regression/with-tensorflow/
...
```bash
cd data-science/regression/with-tensorflow
python -m venv .venv
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/regression/with-tensorflow` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
*Dernière génération : 2026-09-13 · projet `ds-regression-tensorflow`*

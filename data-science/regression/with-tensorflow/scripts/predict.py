"""Exécute des prédictions sur un fichier ou sur un échantillon généré.

Wrapper CLI minimal : il compose la configuration Hydra (mêmes overrides que
`python -m src.main`), force `mode=predict` et délègue à `src.main.main_flow`.

Exemples :
    $ python scripts/predict.py
    $ python scripts/predict.py data.n_samples=1000 seed=7
    $ python scripts/predict.py ++train.epochs=5 log_level=DEBUG
...
def run(overrides: Sequence[str] | None = None) -> int:
    ...
    logger.info("scripts/predict.py | overrides={}", command_overrides or "none")

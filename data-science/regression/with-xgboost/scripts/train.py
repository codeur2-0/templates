"""Entraîne le modèle, l'évalue et sauvegarde les artefacts.

Wrapper CLI minimal : il compose la configuration Hydra (mêmes overrides que
`python -m src.main`), force `mode=train` et délègue à `src.main.main_flow`.

Exemples :
    $ python scripts/train.py
    $ python scripts/train.py data.n_samples=1000 seed=7
    $ python scripts/train.py ++train.epochs=5 log_level=DEBUG
...
def run(overrides: Sequence[str] | None = None) -> int:
    ...
    logger.info("scripts/train.py | overrides={}", command_overrides or "none")

"""Model factory of the **XGBoost** stack.
...
* :func:`build_model` reads the ``model`` node (algorithm, hyper-parameters, seed) and returns a
  :class:`~src.models.model.XGBoostModel` respecting the :class:`~src.models.base.BaseModel`
  contract,
...
def load_model(path: str | Path, *, config: Mapping[str, Any] | None = None) -> BaseModel:
    ...
    if source.suffix.lower() not in SUPPORTED_SUFFIXES:
        msg = (
            f"Unsupported model artefact '{source.name}' for the XGBoost stack "
            f"(expected one of {sorted(set(SUPPORTED_SUFFIXES))})"
        )
        raise ValueError(msg)
    return XGBoostModel.load(source, config=config)

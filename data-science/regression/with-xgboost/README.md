...
> **Domaine** : `data-science` · **Problématique** : `regression` · **Stack** : `XGBoost`
> **Emplacement** : `data-science/regression/with-xgboost/`
...
-> gradient boosting régulé XGBoost avec early stopping natif -> analyse des résidus par segment)
pour un modèle d'évaluation automatisée de prix immobilier, entièrement piloté par Hydra et
...
1. Comprendre le gradient boosting : arbres séquentiels qui corrigent les résidus, shrinkage et régularisation.
2. Utiliser l'early stopping natif (`eval_set` + `early_stopping_rounds`) sans réinventer la boucle d'entraînement.
3. Choisir un objectif de régression (`reg:squarederror`, `reg:absoluteerror`) selon la métrique pilotée.
4. Régulariser un booster (max_depth, min_child_weight, subsample, colsample_bytree, reg_lambda, gamma).
5. Lire une importance de features au gain et connaître ses limites (biais vers les features à forte cardinalité).
6. Comparer les variantes de booster (gbtree, dart, gblinear) et des baselines honnêtes sur un même split.

**Pourquoi XGBoost ici ?**

- Gradient boosting regularisé (L1/L2), très performant sur données tabulaires structurées.
- Early stopping intégré via `eval_set` : surveille la métrique de validation à chaque boosting round.
...
| --- | --- | --- | --- |
| Framework ML | **XGBoost** | joblib>=1.3 | boosting |
...
```text
data-science/regression/with-xgboost/
...
│   ├── models/                  # BaseModel (ABC) + implémentation XGBoost
...
AppConfig (Pydantic)                 ← conf/*.yaml validé et typé
   ...
   │
   ├── XGBoostModel(BaseModel)   ← implémentation XGBoost
...
```bash
cd data-science/regression/with-xgboost
python -m venv .venv
...
python -m src.main mode=train model.params.n_estimators=600
...
- **Reproductibilité** : graine propagée à Python/NumPy/XGBoost, données régénérables à l'identique.
...
| `src/models/` | `BaseModel` (ABC) + implémentation XGBoost | Aucune logique de training loop ici : le modèle expose un contrat. |
...
| --- | --- | --- |
| `ModuleNotFoundError: No module named 'src'` | Exécution depuis un autre répertoire | `cd data-science/regression/with-xgboost` puis `python -m src.main …` (ou `export PYTHONPATH=.`) |
...
| `Could not find a version that satisfies the requirement xgboost` | Index PyPI inaccessible / Python trop ancien | Python ≥ 3.10, `pip install --upgrade pip`, vérifier le proxy |
...
*Dernière génération : 2026-09-17 · projet `ds-regression-xgboost`*

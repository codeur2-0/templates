...
class ProjectConfig(BaseModel):
    ...
    key: str = "ds-recommendation-sklearn"
    name: str = "ecommerce-catalog-recommendation-sklearn"
    domain: str = "data-science"
    problem: str = "recommendation"
    stack: str = "sklearn"
    family: str = "recommendation"
    modality: str = "tabular"
    title: str = "Moteur de recommandation de catalogue e-commerce avec scikit-learn"
    version: str = "1.0.0"
...
class DataConfig(BaseModel):
    ...
    dataset_name: str = "ecommerce_candidate_impressions"
    n_samples: int = Field(default=36000, ge=50)
    seed: int = 42
    formats: list[str] = Field(default_factory=lambda: ["parquet", "csv"])
    raw_dir: Path = Path("data/raw")
    processed_dir: Path = Path("data/processed")
    external_dir: Path = Path("data/external")
    target: str | None = "relevance"
    id_column: str | None = "sample_id"
    time_column: str | None = "session_date"
...
class ModelConfig(BaseModel):
    ...
    name: str = "Gradient boosting par histogrammes (HistGradientBoostingClassifier)"
...
class MetricsConfig(BaseModel):
    ...
    task: Task = "ranking"
    primary: str = "ndcg_at_k"
    secondary: list[str] = Field(
        default_factory=lambda: ["precision_at_k", "recall_at_k", "map_at_k", "hit_rate_at_k"]
    )
    baseline: str = "popularity"
    direction: Literal["maximize", "minimize"] = "maximize"
    min_primary: float | None = 0.45

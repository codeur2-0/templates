...
class TestDataGenerationPipeline:
    ...
    def test_dataset_is_written(self, generation_result: Any, sandbox_paths: ProjectPaths, sandbox_config: AppConfig) -> None:
    ...
    def test_generated_rows_match_configuration(self, generation_result: Any, sandbox_config: AppConfig) -> None:
    ...
    def test_generated_data_passes_the_raw_contract(self, sandbox_paths: ProjectPaths, sandbox_config: AppConfig) -> None:
...
class TestTrainPipeline:
    ...
    def test_training_reports_metrics(self, training_result: Any, sandbox_config: AppConfig) -> None:
    ...
    def test_model_artifacts_are_persisted(self, training_result: Any, sandbox_paths: ProjectPaths) -> None:
...
class TestEvaluationPipeline:
    ...
    def test_metrics_json_is_machine_readable(self, evaluation_result: Any, sandbox_paths: ProjectPaths) -> None:
...
class TestInferencePipeline:
    ...
    def test_predictions_are_written(self, sandbox_config: AppConfig) -> None:
        ...
        candidate = output if output.is_absolute() else ProjectPaths.from_config(sandbox_config.model_dump()).root / output
        assert result.succeeded
        if candidate.exists():
            frame = pd.read_csv(candidate) if candidate.suffix == ".csv" else pd.read_parquet(candidate)
            assert len(frame) >= 1

...
class TestFeatureBuilder:
    ...
    def test_declared_recipes_produce_declared_columns(self, feature_builder: FeatureBuilder, split_frames: Any) -> None:
    ...
    def test_transform_preserves_original_columns(self, feature_builder: FeatureBuilder, split_frames: Any) -> None:
        """Le builder *ajoute* des colonnes, il n'en supprime jamais."""
        enriched = feature_builder.transform(split_frames.train)
        assert set(split_frames.train.columns) <= set(enriched.columns)

    def test_transform_is_deterministic(self, feature_builder: FeatureBuilder, split_frames: Any) -> None:
    ...
    def test_input_frame_is_not_mutated(self, feature_builder: FeatureBuilder, split_frames: Any) -> None:
    ...
    def test_learned_recipes_require_fit(self, sample_frame: pd.DataFrame) -> None:
        ...
        builder = FeatureBuilder([{"type": "bin", "name": "amount_bin", "column": "amount", "bins": 3}])
    ...
    def test_bin_edges_are_learned_on_train(self, sample_frame: pd.DataFrame) -> None:
        ...
        builder = FeatureBuilder([{"type": "bin", "name": "amount_bin", "column": "amount", "bins": 4}])
    ...
    def test_ratio_recipe_is_exact(self, sample_frame: pd.DataFrame) -> None:
        ...
        builder = FeatureBuilder(
            [{"type": "ratio", "name": "ratio", "numerator": "count", "denominator": "denominator_zero"}]
        )
        builder.fit(frame)
    ...
    def test_missing_input_column_gives_an_actionable_error(self, feature_builder: FeatureBuilder, split_frames: Any) -> None:
    ...
    def test_select_feature_columns_excludes_drops(self, split_frames: Any, app_config: Any) -> None:
        """Le sélecteur exclut la cible et les colonnes non modélisables."""
        columns = select_feature_columns(
            split_frames.train, drop_columns=app_config.data.drop_columns, target=app_config.data.target
        )
...
class TestPreprocessingPipeline:
    ...
    def test_feature_names_out_matches_the_matrix(self, preprocessing: PreprocessingPipeline, matrices: dict[str, Any]) -> None:
    ...
    def test_transform_before_fit_is_refused(self, sample_frame: pd.DataFrame, app_config: Any) -> None:
    ...
    def test_scaler_statistics_come_from_train_only(self, preprocessing: PreprocessingPipeline, matrices: dict[str, Any]) -> None:
        """Anti-fuite : le test est centré/réduit avec les statistiques du **train**."""
        numeric_outputs = [
            column for column in matrices["X_train"].columns if pd.api.types.is_numeric_dtype(matrices["X_train"][column])
        ]
    ...
    def test_categorical_columns_are_encoded(self, preprocessing: PreprocessingPipeline, matrices: dict[str, Any]) -> None:
    ...
    def test_report_documents_the_transformations(self, preprocessing: PreprocessingPipeline) -> None:
    ...
    def test_save_load_roundtrip_is_lossless(self, preprocessing: PreprocessingPipeline, matrices: dict[str, Any], tmp_path: Any) -> None:
...
class TestCustomTransformers:
    ...
    @pytest.mark.parametrize(
        ...
        ids=["column_selector", "type_caster", "outlier_clipper", "log1p", "rare_category", "scaler"],
    )
    def test_every_transformer_is_cloneable(self, transformer: Any, sample_frame: pd.DataFrame) -> None:
    ...
    def test_log1p_transform_is_exact(self, sample_frame: pd.DataFrame) -> None:
        ...
        np.testing.assert_allclose(transformer.transform(sample_frame)["amount"].to_numpy(), expected)

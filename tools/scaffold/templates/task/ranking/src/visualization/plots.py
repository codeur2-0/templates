...
class RankingPlots:
    ...
    def baselines_comparison(
        ...
        for index, row in enumerate(table.itertuples()):
            ...
            bars = axis.bar(
                ...
                color=colours.get(str(row.scorer), None),
                edgecolor="white",
            )
            for bar, value in zip(bars, values, strict=True):
                if np.isnan(value):
    ...
    def coverage_vs_cutoff(
        ...
        right = left.twinx()
        right.plot(
            cutoffs, curves["herfindahl"], marker="s", markersize=4, color=COLOR_CEILING
        )
    ...
    def segment_breakdown(
        self, result: Any, *, name: str = "segment_breakdown.png"
    ) -> Path | None:
        ...
        if not axes:
            return None
        fig, gridspec = plt.subplots(
            len(axes), 1, figsize=(9.6, 2.5 * len(axes)), squeeze=False
        )
    ...
    def stock_and_margin(
        self, result: Any, *, name: str = "stock_and_margin.png"
    ) -> Path | None:
        ...
        if "marge_pct" in published.columns:
            ...
            if not margins.empty:
                ...
                axes[1].set_title(
                    f"Marge des articles publiés (moyenne {margins.mean() * 100:.1f} %)", fontsize=10
                )
    ...
    def backtest_stability(
        ...
        amplitude = _safe_float(dispersion.get("amplitude"), float(np.nanmax(values) - np.nanmin(values)))
        axis.set_title(
            f"Stabilité temporelle du NDCG@{_top_k(result)} — amplitude {amplitude:.3f}"
        )
    ...
    # ------------------------------------------------------------------ errors ----------
    def worst_misrankings(
        self, result: Any, *, name: str = "worst_misrankings.png"
    ) -> Path | None:
        ...
        bars = axes[0].barh(labels[::-1], displayed["cout_ndcg"].to_numpy()[::-1], color=colours[::-1])
        ...
        axes[0].set_title(f"Les {len(displayed)} mauvais classements les plus coûteux", fontsize=10.5)

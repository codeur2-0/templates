...
class ClusteringPlots:
    ...
    def k_selection(self, result: Any, *, name: str = "k_selection.png") -> Path | None:
        ...
        left = axes[0]
        left.plot(
            table["k"], table["silhouette"], marker="o", color="#005f73", label="silhouette"
        )
    ...
    def silhouette_distribution(
        ...
        axis.set_title(
            f"Silhouette individuelle — {negative:.1%} des clients "
            "plus proches d'un autre groupe"
        )
    ...
    def cluster_sizes(self, result: Any, *, name: str = "cluster_sizes.png") -> Path | None:
        ...
        colors = [
            "#ae2012" if value < MIN_CLUSTER_SHARE * 100.0 else "#005f73" for value in shares
        ]
    ...
    def confidence_breakdown(
        ...
        order = [level for level in ("élevée", "moyenne", "faible", "indéterminée")

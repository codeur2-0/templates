...
class Evaluator:
    ...
    def evaluate(
        ...
        *,
        ...
        extras: dict[str, Any] = {
            ...
            "centroid_inertia": float(np.mean(np.square(distances))) if len(distances) else float("nan"),
        }
    ...
    def compare_to_baseline(self, X: pd.DataFrame, *, baseline: str = "random_assignment") -> dict[str, float]:
        ...
        if baseline == "single_cluster":
            ...
            values = calculator.evaluate(
                MetricInputs(y_true=None, y_pred=labels, X=features)
            )
    ...
    def save_metrics(self, result: EvaluationResult, path: str | Path | None = None) -> Path:
        ...
        destination = Path(path) if path is not None else (
            self.paths.metrics_dir / "clustering_metrics.json"
        )
    ...
    def _compute_metrics(self, features: np.ndarray, assignments: np.ndarray) -> dict[str, float]:
        ...
        calculator = MetricCalculator(task=self.task, metrics=self.metric_names)
        values = calculator.evaluate(
            MetricInputs(y_true=None, y_pred=assignments, X=features)
        )
    ...
    def _centroid_distances(
        ...
        index = {int(label): position for position, label in enumerate(sorted(np.unique(assignments)))}
        ...
        if context is not None and len(context) == len(frame):
            kept = [
                ...
                for name in dict.fromkeys(
                    [*self.context_columns, LATENT_COLUMN, OUTCOME_COLUMN, *self._raw_columns(context)]
                )

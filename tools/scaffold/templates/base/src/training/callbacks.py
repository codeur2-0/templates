...
class ProgressBarCallback(BaseCallback):
    ...
    def on_train_begin(self, context: CallbackContext) -> None:
        ...
        self._bar = tqdm(
            total=context.epochs, desc=f"Training {context.model_name}", leave=self.leave, unit="epoch"
        )
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def on_epoch_end(self, context: CallbackContext) -> None:
        ...
        if value is None:
            logger.debug("EarlyStopping: metric '{}' not in logs {}", self.monitor, sorted(context.logs))
            return
        value = float(value)
...
class MetricThresholdCallback(BaseCallback):
    ...
    def on_epoch_end(self, context: CallbackContext) -> None:
        ...
        if value is None:
            return
        ok = float(value) >= self.threshold if self.mode == "max" else float(value) <= self.threshold
